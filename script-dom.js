...
const createMarkup = quantity => {
  ...
  for (let i = 0; i < quantity; i++) {
    ...
    blocksItem.className = "blocks-item";
    blocksItem.innerText = i + 1;

    markup += blocksItem.outerHTML; 
  }
...
colorSelector.addEventListener("change", e =>{
  ...
  const items = blocksContainer.querySelectorAll(".blocks-item");
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    colorBlocks(currentBlock, colorToApply);
  }
})
